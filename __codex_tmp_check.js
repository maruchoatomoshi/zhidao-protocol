...
function updateRaidUI(joinedCount, target = RAID_CONFIG.minPlayers) {
    ...
    if (!currentLabel || !maxLabel || !progressBar) return;
    ...
    if (safeCount >= safeTarget) {
        ...
        progressBar.style.background = 'linear-gradient(90deg, var(--red), #ff4400)';
        progressBar.style.boxShadow = '0 0 10px var(--red-dim)';
        progressBar.style.animation = '';
    }
}
...
function prepareRaidVisual() {
    ...
    bossImg.onerror = function() {
        ...
        placeholder.style.borderRadius = currentTheme === 'genshin' ? '20px' : '50%';
