...
function _buildIntroNovelScenes() {
  ...
  return {
    ...
    s2:  { who: 'system', text: 'Ты — новый оператор ZHIDAO Protocol. С подключением, username', next: 's3' },
    s3:  { who: 'system', text: 'До старта поездки осталось немного времени. Некоторые разделы уже активны, но время некоторых ещё не пришло', next: 's4' },
    ...
    s17: { who: 'yulia', text: 'Сейчас я на расстоянии, на связи дистанционно, но всё равно слежу за вами и помогаю издалека', next: 's18' },
    ...
    s19h: { who: 'yulia', text: 'Если что-то про комфорт и нервы — это не ко мне, это к тому, кто рядом с вами физически, а не на расстоянии', next: 's20' },
    ...
    s24h: { who: 'liza', text: 'Передаю дальше слово дргуим нашим замечательным сопровождающим', next: 's25' },
    ...
    sO1r: { who: 'olga', text: 'Привет! Елизавета Сергеевна уже всё объяснила про самочувствие, мы — про дела', next: 'sO1' },
    sO1:  { who: 'olga', text: 'Я Ольга Михайловна. Стирка, вода, бытовые вопросы: мы всегда рядом, чтобы помочь', next: 'sO2' },
    sO2:  { who: 'wunan', text: '大家好, я Ву Нан', next: 'sO3' },
    sO3:  { who: 'wunan', text: 'Если что-то по расписанию дня, по комнатам или мелким делам — спрашивайте, не стесняйтесь. Комфортно вам, а значит и нам тоже', next: 'sO4' },
    sO4:  { who: 'olga', text: 'Это не самая заметная часть Протокола, но именно на ней держится комфорт каждого дня', next: 'sO5' },
    sO5:  { who: 'system', text: 'Узел поддержки подключён', next: 's26' },

    s26: { who: 'system', text: 'Внешний контур отвечает...', next: 's27' },
    s27: { who: 'system', text: 'BEIJING NODE // ожидание подтверждения', next: 's28r' },
    s28r: { who: 'tianhao', text: '大家好，позвольте представиться', next: 's28' },
    ...
    s30: { who: 'tianhao', text: 'Там будут улицы, древние памятники, столовые, магазины, фразы, которые нужно понять быстро, и ситуации, которых нет в учебнике', next: 's31' },
    ...
    s32: { who: 'tianhao', text: 'Слушай подсказки, повторяй, используй свои навыки. Китайский нужен не только на уроке', next: 's32h' },
    ...
    s36r: { who: 'architect', text: 'Дисциплина, связь, равновесие, поддержка, китайский — все узлы на месте. Я просто завершаю круг', next: 's36' },
    s36: { who: 'architect', text: 'Я Марк Альбертович', next: 's37' },
    s37: { who: 'architect', text: 'В системе я Архитектор, который выстроил этот протокол: профиль, рейтинг, дневник, задания, магазин, события и всё, что будет сопровождать поездку', next: 's38' },
    s38: { who: 'architect', text: 'Но важно понять сразу: это не просто игра. Вы получаете систему, соответствуете её правилам, и система даёт вам награду', next: 's39' },
    ...
    s40b: { who: 'architect', text: 'Ошибки будут. Для этого рядом мы и остальные операторы. Главное — не скрываться от системы и не пытаться ломать правила', next: finalStart },
    ...
    fA4: { who: 'architect', text: 'Остальное появится совсем скоро. Просто будь на связи. Если вдруг ты найдёшь ошибки и баги, ты знаешь, с кем связаться', next: 'fA5' },
    fA5: { who: 'system', text: 'PRE-LAUNCH MODE активен', next: 'fA6' },
    fA6: { who: 'architect', text: 'Протокол запущен. Нейроподключение выполнено. Приглашаем тебя стать частью лучшего лета в Китае. Удачи, оператор!', next: null },
    ...
    fB4: { who: 'architect', text: 'Протокол не требует чего-то неординарного. Он требует участия', next: 'fB5' },
    fB5: { who: 'system', text: 'BEIJING MODE активен', next: 'fB6' },
    fB6: { who: 'architect', text: 'Протокол запущен. Нейроподключение выполнено. Приглашаем тебя стать частью лучшего лета в Китае. Удачи, оператор!', next: null }
  };
}
