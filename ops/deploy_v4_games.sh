...
target=4b5cb889128129db0e9125a0abfe0294a19e7ea1
...
for attempt in range(20):
    try:
        with urlopen(base + '/api/v4/health', timeout=2) as r:
            health = json.load(r)
        assert health['status'] == 'ok' and health['schema_version'] == 16
        break
    except Exception:
        if attempt == 19:
            raise
...
print('Local smoke: schema 16; cases rules and release assets 200; personal API 401')
PY
