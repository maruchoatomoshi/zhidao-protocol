...
target=5f17f4c915ac8c7350a1de3ba7cff83e46089964
...
for attempt in range(20):
    try:
        with urlopen(base + '/api/v4/health', timeout=2) as r:
            health = json.load(r)
        assert health['status'] == 'ok' and health['schema_version'] == 18
        break
    except Exception:
        if attempt == 19:
            raise
...
print('Local smoke: schema 18; cases rules and release assets 200; personal API 401')
PY
