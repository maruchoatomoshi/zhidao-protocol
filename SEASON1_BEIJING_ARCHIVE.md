# Season 1 — Beijing archive

This is the season-1 (Beijing) `CLAUDE.md`, preserved verbatim from tag
`season1-beijing` (`550ff15`). It is kept as the historical record of the
Telegram Mini App: its incidents, mechanics, the МЮ feature backlog and the
"Blue Horizon" lore draft.

**Read this as history, not as instructions.** Two things in it are no longer
true:

1. **The production server is dead.** `hk.marucho.icu` and its database are
   gone (confirmed 2026-09-03). Every server command below — `diff
   /root/zhidao_api.py`, `systemctl restart zhidao_api.service`, `journalctl
   -u zhidao_api.service`, the backup and recovery runbooks — refers to a
   machine that no longer exists. Do not run them and do not ask the user to.
   The "Current Priority: backend is stable on the server" section is
   obsolete.
2. **The active line is Hainan V4**, not this app. See `CLAUDE.md` for current
   working rules, and `V4_DECISIONS.md` for scope.

What is still useful here: the SQLite/WAL failure modes (also summarised in
`CLAUDE.md`, and still relevant because V4 is still FastAPI + SQLite), the
mechanics and terminology of season 1, the МЮ backlog as a source of product
ideas, and the "Blue Horizon" narrative draft.

The season-1 frontend is feature-frozen in code: `js/config.js` has
`APP_FEATURE_FREEZE_ENABLED = true` since 2026-07-26.
